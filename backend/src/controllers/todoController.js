...
export const createTodo = async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ message: '투두 생성 실패', error: error.errors.message });
  }
};
...
export const updateTodo = async (req, res) => {
  try {
    const { id } = req.params;
    const { task, category, priority, dueDate, isCompleted } = req.body;
    ...
    const updatedTodo = await Todo.findByIdAndUpdate(
      id,
      { task, category, priority, dueDate, isCompleted },
